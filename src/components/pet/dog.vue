  ...
  <div>
    ...
    <div v-if="tableData.length!=0">
      <div class="dog-list-box" @click="goDetail(item)" v-for="item in tableData">
        ...
        <el-image class="dog-picture" :src="item.picture" fit="fill"></el-image>
        <span class="dog-title">{{item.name}}</span>
        <div class="dog-info-box">
          <span class="dog-info">{{item.variety}}</span>
          <span class="dog-info">{{item.age}}</span>
        </div>
        <div class="dog-info-box">
          <span class="dog-info" v-if="item.status=='saled'">售出</span>
...
export default {
  ...
  methods: {
    ...
    goQuery() {
      // const params =this.$route.path.slice(5)
      // console.log(params)
      const query = this.makeQuery();
      this.axios
          .get("/pets/typename/dog", {
